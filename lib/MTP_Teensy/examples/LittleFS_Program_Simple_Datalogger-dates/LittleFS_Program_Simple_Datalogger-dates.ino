...
// NOTE: This option is only available on the Teensy 4.0, Teensy 4.1 and Teensy
// Micromod boards.
// With the additonal option for security on the T4 the maximum flash available
// for a
// program disk with LittleFS is 960 blocks of 1024 bytes
#define PROG_FLASH_SIZE                                                        \
  1024 * 1024 * 1 // Specify size to use of onboard Teensy Program Flash chip
  ...
  while (true) {
    ...
    if (entry.isDirectory()) {
      ...
      Serial.printf("    Created: %d-%02d-%02d %02d:%02d:%02d\n",
                    FS_YEAR(c_date), FS_MONTH(c_date), FS_DAY(c_date),
                    FS_HOUR(c_time), FS_MINUTE(c_time), FS_SECOND(c_time));
